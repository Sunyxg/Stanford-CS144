...
// 用bind将一个套接字绑定到一个指定的地址，通常用于监听/接收
...
// 可以是`SHUT_RD`、`SHUT_WR`或`SHUT_RDWR`
...
// set socket option
// 设置套接字选项
//! \param[in] level The protocol level at which the argument resides 该参数所在的协议级别
//! \param[in] option A single option to set 设置单一选项
//! \param[in] option_value The value to set 选项设置的值
