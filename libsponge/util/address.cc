...
// 将Raw转换为`const sockaddr *`
Address::Raw::operator const sockaddr *() const { return reinterpret_cast<const sockaddr *>(&storage); }

//! \param[in] addr points to a raw socket address 指向一个原始的套接字地址
//! \param[in] size is `addr`'s length 是addr的长度
...
// 构建getaddrinfo和getnameinfo故障的错误类别
...
//! \param[in] node is the hostname or dotted-quad address 是hostname或者ip地址
//! \param[in] service is the service name or numeric string 服务名称或端口号
...
// 为[getaddrinfo(3)]构建一个addrinfo(hints)
